 ...
 * RIGHT BUTTON - Press and relese to change mode (difficulty) - Press and hold
 * to toggle sound
 *
 * Also, from standby....
...
void playPong() {
  ...
  while (stopAnimate == 0) {
    ...
    while (1) {
      ...
      player += 1; // NOTICE: Always goes down!
      if (player > 48)
        player = 48;
      if (player < 0)
        player = 0;
      ...
      if (mode == 1 || mode == 3 || mode == 4) {
        if (waitCount >= 3) {
          ...
          if (perturbation > pFactor)
            perturbation = pFactor - 2;
          if (perturbation < pFactor * -1)
            perturbation = (pFactor * -1) + 2;
        }
      ...
      if ((actualy + vdir < 63 && vdir > 01) ||
          (actualy - vdir > 6 && vdir < 0)) {
        bally += vdir;
      } else {
        vdir = vdir * -1;
      }
...
// Useless method; fillScreen may do the same work...
