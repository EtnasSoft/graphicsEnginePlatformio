...
NEWLEVEL:
  if (LEVELSPEED > 10) {
    ...
    if ((LEVELSPEED == 160) || (LEVELSPEED == 120) || (LEVELSPEED == 80) ||
        (LEVELSPEED == 40) || (LEVELSPEED == 10)) {
      if (LIVE < 3) {
        LIVE++;
        for (t = 0; t <= 4; t++) {
          Sound(80, 100);
...
RESTARTLEVEL:
  ...
  while (1) {
    ...
    if (INGAME) {
      ...
      } else if ((analogRead(A3) > 500) && (analogRead(A3) < 750)) {
    ...
    if (Frame % 2 == 0) {
      ...
      if (INGAME == 1) {
        for (uint8_t t = 0; t <= 139; t = t + 2) {
          Sound(pgm_read_byte(&Music[t]) - 8,
                (pgm_read_byte(&Music[t + 1]) - 100));
        }
        INGAME = 2;
      }
    } else {
...
#define ymin(I) ((Sprite[I].y * 8) + Sprite[I].Decalagey)
  if ((INGAME)) {
    for (uint8_t t = 1; t <= 4; t++) {
      if ((xmax(0) < xmin(t)) || (xmin(0) > xmax(t)) || (ymax(0) < ymin(t)) ||
          (ymin(0) > ymax(t))) {
      } else {
        if (Gobeactive) {
          if (Sprite[t].guber != 1) {
...
void RefreshCaracter(PERSONAGE *Sprite) {
  ...
  for (uint8_t t = 0; t <= 4; t++) {
    ...
    if (t == 0) {
      ...
      if ((Sprite[t].guber == 1) && (Sprite[t].x >= 74) &&
          (Sprite[t].x <= 76) && (Sprite[t].y >= 2) && (Sprite[t].y <= 4)) {
        Sprite[t].guber = 0;
      }
      if (Frame % 2 == 0) {
        if (Sprite[t].anim < 1) {
...
uint8_t CheckCollisionWithBack(uint8_t SpriteCheck, uint8_t HorVcheck,
                               PERSONAGE *Sprite) {
  uint8_t BacktoComp;
  if (HorVcheck == 1) {
...
uint8_t RecupeBacktoCompH(uint8_t SpriteCheck, PERSONAGE *Sprite) {
  ...
  if (Sprite[SpriteCheck].DirectionH == 0) {
    ...
    for (uint8_t t = 0; t <= 6; t++) {
      if ((((Sprite[SpriteCheck].y) * 128) + (Sprite[SpriteCheck].x + t) >
           1023) ||
          (((Sprite[SpriteCheck].y) * 128) + (Sprite[SpriteCheck].x + t) < 0)) {
        TempPGMByte = 0x00;
      } else {
        TempPGMByte = (pgm_read_byte(&back[((Sprite[SpriteCheck].y) * 128) +
...
#define CHECKCOLLISION ((RECUPE) & (TempPGMByte))
    ...
    for (uint8_t t = 0; t <= 6; t++) {
      if (((((Sprite[SpriteCheck].y + tadd) * 128) +
            (Sprite[SpriteCheck].x + t)) > 1023) ||
          ((((Sprite[SpriteCheck].y + tadd) * 128) +
            (Sprite[SpriteCheck].x + t)) < 0)) {
        TempPGMByte = 0x00;
      } else {
        TempPGMByte =
            pgm_read_byte(&back[((Sprite[SpriteCheck].y + tadd) * 128) +
                                (Sprite[SpriteCheck].x + t)]);
      }
  ...
  for (y = 0; y < 8; y++) {
    ...
    for (x = 0; x < 128; x++) {
      if (render0_picture1 == 0) {
        if (INGAME) {
          SSD1306.ssd1306_send_byte(
              background(x, y) | SpriteWrite(x, y, Sprite) |
              DotsWrite(x, y, Sprite) | LiveWrite(x, y) | FruitWrite(x, y));
        } else {
          SSD1306.ssd1306_send_byte(
              0xff - (background(x, y) | SpriteWrite(x, y, Sprite)));
        }
...
uint8_t DotsWrite(uint8_t x, uint8_t y, PERSONAGE *Sprite) {
  ...
  if (mem1 != 0b00000000) {
    ...
    if (checkDotPresent(dotscount) == 0b00000000) {
      ...
      if ((Sprite[0].type == PACMAN) &&
          ((Sprite[0].x < x) && (Sprite[0].x > x - 6)) &&
          (((Sprite[0].y == y) && (Sprite[0].Decalagey < 6)) ||
           ((Sprite[0].y == y - 1) && (Sprite[0].Decalagey > 5)))) {
...
uint8_t SplitSpriteDecalageY(uint8_t decalage, uint8_t Input,
                             uint8_t UPorDOWN) {
  if (UPorDOWN) {
...
uint8_t SpriteWrite(uint8_t x, uint8_t y, PERSONAGE *Sprite) {
  ...
  while (1) {
    if (Sprite[var1].y == y) {
      AddBin = AddBin | SplitSpriteDecalageY(
                            Sprite[var1].Decalagey,
                            return_if_sprite_present(x, Sprite, var1), 1);
    } else if (((Sprite[var1].y + 1) == y) && (Sprite[var1].Decalagey != 0)) {
      AddBin = AddBin | SplitSpriteDecalageY(
                            Sprite[var1].Decalagey,
                            return_if_sprite_present(x, Sprite, var1), 0);
    }
    var1++;
    if (var1 == 5) {
      break;
...
uint8_t return_if_sprite_present(uint8_t x, PERSONAGE *Sprite,
  ...
  if ((x >= Sprite[SpriteNumber].x) && (x < (Sprite[SpriteNumber].x + 8))) {
    ...
    return pgm_read_byte(&caracters[((x - Sprite[SpriteNumber].x) +
                                     (8 * (Sprite[SpriteNumber].type * 12))) +
                                    (Sprite[SpriteNumber].anim * 8) +
                                    (Sprite[SpriteNumber].DirectionAnim * 8) +
                                    (ADDgobActive) + (ADDGober)]);
  }
  return 0;
}
