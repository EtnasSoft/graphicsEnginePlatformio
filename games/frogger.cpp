 ...
 *
 *  Special thanks to @senkunmusahi, who created the artwork bitmaps in the game
 * using
 * https://www.riyas.org/2013/12/online-led-matrix-font-generator-with.html
 *
 *  Inspired by http://webboggles.com/ and includes some code from the
 * #AttinyArcade games on that site The code that does not fall under the
 * licenses of sources listed below can be used non commercially with
 * attribution. This software is supplied without warranty of any kind.
 *
 *  Controls:
 ...
 *
 *  HIGHLY RECOMMENDED:
 *  On custom hardware (see schematic in folder where you found this file) there
 * is an additional button to move frog forward
 *
 *  Also, from standby....
 ...
 *  **Note that this highly size-optimised version requires modified library
 * functions (which are in this source code file) and a modified font header
 *
...
// Make click delay an even number - it gets halved and then used in an integer
// comparison
...
// Drawing functions - adapted from those at
...
// Other generic functions for games (both originated in code from
// webboggles.com and the sleep code is by Matthew Little - see above)
void beep(int, int);
...
// Global variables - yes I know all these global vars is a lazy way to code but
// it makes it easier to prevent stack overflows when you're working with 512
// bytes! Most of these are initialised in the main game function
// (playFrogger())
int watchDog; // Counts drawing cycles so I can shut the game down if there's
              // inactivity - battery saver!
boolean stopAnimate; // this is set to 1 when a collision is detected
int lives; // Lives in the game - this can go negative to end the game, which is
           // why it's a signed variable
bool
    frogDocks[5]; // Tracks which frog docks are full (at the top of the screen)
bool flipFlop; // Used in routines that flip-flop between two states (left and
               // right)
bool flipFlopShift; // Same as previous one
byte frogColumn;    // Column location of frog (there are 16 altogether)
byte frogRow; // Row locaiton of frog (there are 8, but 0 is the frog docks at
              // the top and 7 is the start row)
byte frogLeftLimit; // Left limit of frog travel on start row (changes as digits
                    // in score increases)
byte frogRightLimit; // Right limit of frog travel on start row (changes as
                     // lives decrease as there's then more space)
...
int moveDelay; // How long to wait until the next movement of logs etc - changes
               // as levels increase to make the game go faster
...
// Bitmaps created by @senkunmusahi using
// https://www.riyas.org/2013/12/online-led-matrix-font-generator-with.html
...
// Opening artwork created by @senkunmusahi using
// https://www.riyas.org/2013/12/online-led-matrix-font-generator-with.html
static const byte titleBmp[] PROGMEM = {
    ,
...
void loop() {
  ...
  // The lower case character set is seriously compromised because I've had to
  // truncate the ASCII table to release space for executable code. There is no
  // z in the table as this isn't used anywhere in the text here and most of the
  // symbols are also missing for the same reason (see my hacked version of
  // font6x8.h - font6x8AJ.h for more detail)
  ...
  ssd1306_char_f6x8(0, 7, "zsenkunmusashi"); // see comments above - f has been
                                             // replaced by @ in the ASCII table
...
void ssd1306_send_data_stop(void) { ssd1306_xfer_stop(); }
...
void playFrogger() {
  ...
  lives = 2;
  frogRightLimit = 12;
  watchDog = 1; // we use this to see if there's been movement - it's only ever
                // zero when the frog has just moved!
  ...
  while (lives >= 0) {
    ...
    if (watchDog >= 500)
      lives = -1; // Stop the game if nothing's happening - maybe triggered in
                  // someone's pocket so this is to save battery!
    ...
    // Handle input from 'jump' button (the other two buttons are captured in
    ...
    if (moveLeft == 1 && millis() > clickBase + CLICKDELAY / 2) {
      ...
      if (digitalRead(2) == HIGH)
        ...
        if ((frogRow == 7 && frogColumn > frogLeftLimit) ||
            (frogRow < 7 && frogColumn > 0)) {
          frogColumn--;
        } else if (frogRow < 7)
          stopAnimate = 1;
    ...
    if (moveRight == 1 && millis() > clickBase + CLICKDELAY / 2) {
      ...
      if (digitalRead(0) == HIGH)
        ...
        drawFrog(0, 0); // delete the frog
        // move the frog, checking it isn't jumping off the edge of the screen
        if ((frogRow == 7 && frogColumn < frogRightLimit) ||
            (frogRow < 7 && frogColumn < 14)) {
          frogColumn++;
        } else if (frogRow < 7)
          stopAnimate = 1;
    ...
    if (moveForward == 1) {
      ...
      if (frogRow > 1) {
        frogRow--;
        // Correct for the skew in frog position created by the blockShift
        // scrolling parameter
        if (frogRow == 3 && blockShiftL < 4)
          frogColumn--;
        if (frogRow == 2 && blockShiftR + blockShiftL < 5)
          frogColumn++;
        if (frogRow == 1 && blockShiftR + blockShiftL < 5)
          frogColumn--;

      } else {
        // frog is at the docks!
        if (blockShiftL < 4 && frogColumn < 15)
          frogColumn++; // account for skew due to block shifting
      ...
      if (dockedFrogs >= 5) {
        ...
        levelUp(level);
        if (moveDelay > 99)
          moveDelay -= 100; // make the game speed up
    ...
    if (watchDog == 0 && stopAnimate == 0) {
      watchDog = 1; // set to something other than zero so this routine doesn't
                    // run again
      // redraw the frog
      drawFrog(frogMode, 0);
      // redraw the screen
    ...
    if (stopAnimate != 0) {
      ...
      frogRightLimit++; // there's one less frog drawn on right so you can move
                        // a bit further across (if you really want to!)
...
void checkCollision(void) {
  if (frogRow > 0 && frogRow < 4 && grid[frogRow - 1][frogColumn] == 0)
    stopAnimate = 1; // the frog has fallen in the river
  if (frogRow > 0 && frogRow < 4 && grid[frogRow - 1][frogColumn] > 9)
    stopAnimate = 1; // the frog has stepped on a croc
  if ((frogRow < 7 && frogRow > 3) && (grid[frogRow - 1][frogColumn] != 0 ||
                                       grid[frogRow - 1][frogColumn - 1] != 0))
    stopAnimate = 1; // the frog has been hit by a vehicle
}
...
void initScreen(void) {
  int initCounter[6] = {
      3, 2, 4,
      2, 2, 3}; // the length of the objects on each row - doesn't change
  int gapCounter[6] = {-2, -3, -4, -4,
                       -3, -5}; // the gaps between objects - change with levels
                                // to make it harder as you go thru the game
  int counter[6];               // used to hold the gap data
  byte stepMode = 0; // which component of the object are we drawing (they all
                     // have three - a start a middle and an end)
  byte stepShift =
      0; // offset to shift up to the different objects in the array
  byte crocStartColumn = 0; // column at which to stop drawing crocs - is zero
                            // at start hence no crocs!
  ...
  if (level < 3) {
    gapCounter[4] = -6; // make it easier for levels less than 3 by increasing
                        // the gap in the cars on this row
  }
  if (level < 4) {
    gapCounter[3] = -7;
  }
  if (level > 4) {
    for (byte incr = 1; incr < 3; incr++) {
      gapCounter[incr]--; // increase the gaps between the logs for levels over
                          // 4
    }
  }
  ...
  if (level > 2)
    crocStartColumn = 5; // one croc appears at level 3 and above
  if (level > 6)
    crocStartColumn = 9; // two croc appear at level 7 and above

  // Initialise the counters
  for (byte incr = 0; incr < 6; incr++)
    counter[incr] = initCounter[incr];
    ...
    for (byte col = 0; col < 15; col++) {
      if (counter[row] > 0) {
        if (14 - row > counter[row]) {
          if (counter[row] == 1)
            if (stepMode == 1)
              stepMode = 2; // the next space is blank and we are drawing the
                            // middle - draw the end!
          if (row > 2)
            stepShift = 3;
          else
            stepShift = 0; // shift up to the trucks in the array
          if (row == 4)
            stepShift =
                9; // shift up to the cars in the array - also theres no middle

          if (row > 0) {
            grid[row][col] =
                4 + stepMode +
                stepShift; // if you are on any row but the first - draw
                           // whatever is appropriate from the bitmaps
          } else if (col >= crocStartColumn) {
            grid[row][col] =
                4 + stepMode + stepShift; // if you're on row zero (top row of
                                          // logs) and you are above where crocs
                                          // should be drawm, draw logs ...
          } else
            grid[row][col] = 10 + stepMode; // .. otherwise draw crocs
          if (stepMode == 0)
            stepMode =
                1; // we've drawn the left side now switch to central sections
          if (stepMode == 2)
            stepMode = 0; // we've drawn the end, now reset
        }
      }
      counter[row]--; // decrement the counter
      if (counter[row] <= gapCounter[row]) {
        counter[row] =
            initCounter[row]; // if we have gone negative enough to account for
                              // the gaps - reset the counter and start again
      }
    }
  }
}
...
void drawFrog(byte mode, bool frogDead) {
  if (frogRow > 6 || frogRow < 1 ||
      frogDead ==
          1) { // don't draw the frog when it's on the road or on logs - because
               // they are moving, that's handled in the main drawing routine
               // below- exception is when you are animating frog death
    if (frogRow == 1 ||
        frogRow == 3) { // these allow for the blocks being shifted when
                        // animating the frog death on rows with logs
...
void drawGameScreen(byte mode) {
  ...
  for (byte row = 0; row < 6; row += 2) {
    if (row >= 0 && row < 3)
      ...
      inverse = 0; // draw everything (except the frog) in inverse video on the
                   // river rows (0,1,2)
    ssd1306_setpos(
        0, row + 1); // +1 because row 0 here is actually row 1 on the screen
    ssd1306_send_data_start();
    for (byte incr = 0; incr < 7 - blockShiftL; incr++)
      if (grid[row][15] == 0) { // cover the tiny bit to the far left of the
                                // screen up to wherever the main blocks will be
                                // drawn (depends on how far they are shifted)
        sendByte(0, inverse);   // draw an empty 8-bit line if there's nothing
                                // wrapping around
      } else {
        sendByte(
            pgm_read_byte(&bitmaps[grid[row][15] - 1][1 + blockShiftL + incr]),
            inverse); // pick the correct bit of whatever is wrapping from the
                      // right of the screen
      }
    for (byte col = 0; col < 15; col++) {
      if (frogRow == row + 1 && frogColumn == col && frogRow < 4 &&
          frogRow > 0) {
        sendBlock(mode, 0); // if we are in a location with the frog, and it's
                            // on the logs, draw it - never invert it (hence
                            // zero as second parameter here)
      } else if (stopAnimate == 0 && frogRow == row + 1 &&
                 frogColumn == col + 1 && frogRow > 3 &&
                 frogRow < 7) { // frog is amongst the cars and needs drawing
        for (byte incr = 0; incr < blockShiftL; incr++)
          sendByte(0, 0);   // draw the blank space up to the frog
        sendBlock(mode, 0); // draw frog
        for (byte incr = 0; incr < 7 - blockShiftL; incr++)
          sendByte(0, 0); // draw the blank space after the frog
        col++;            // we've now drawn two columns so increment
      } else {
        sendBlock(grid[row][col], inverse); // draw the correct object for this
                                            // space - it's not a frog ;)
      }
    }
  ...
  if (frogColumn == 0)
    drawFrog(mode, 1); // this covers the exceptional case where the frog is in
                       // the far left colum, in which case the normal routine
                       // can't draw it when it's on the road

  // Draw objects going right - see comments above, works in basically the same
  // way
    ...
    for (byte incr = 0; incr < blockShiftR; incr++)
      if (grid[row][15] == 0)
        ...
        sendByte(pgm_read_byte(
                     &bitmaps[grid[row][15] - 1][incr + (8 - blockShiftR)]),
                 inverse);
    for (byte col = 0; col < 15; col++) {
      if (frogRow == row + 1 && frogColumn == col && frogRow < 4 &&
          frogRow > 0) {
        sendBlock(mode, 0);
                 ...
                 frogColumn == col + 1 && frogRow > 3 && frogRow < 7) {
...
// Send one block of 8 bytes to the screen - inverse means inverse video, for
// the river section
void sendBlock(byte fill, bool inverse) {
  for (int incr = 0; incr < 8; incr++) {
    if (fill > 0) {
      if (inverse == 0)
        ssd1306_send_byte(pgm_read_byte(&bitmaps[fill - 1][incr]));
      else
        ssd1306_send_byte(~pgm_read_byte(&bitmaps[fill - 1][incr]));
    } else if (inverse == 0)
      ssd1306_send_byte(0);
...
// Move all the items on the game screen (wrapping at the ends) and check for
// frog dropping off the end of the screen
void moveBlocks(void) {
  int direct = 0;
  ...
  for (byte row = 0; row < 6; row++) {
    // Move the frog along and check to see whether it's gone off the screen, in
    // which case it dies
