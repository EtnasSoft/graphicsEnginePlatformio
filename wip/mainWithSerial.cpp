...
unsigned int player = numberOfSprites - 1; // Position of the player in the listObjects
...
void debug(const char* debugString, bool pause = false) {
  initTXPin();
  Serial.print(debugString);
  if (pause) buttonWait();
}

void debugln(const char* debugString, bool pause = false) {
  initTXPin();
...
void printDebug() {
  ...
  writeUnsignedByte(playerMove);
  Serial.print(" | Player Action: ");
  writeUnsignedByte(playerAction);
  Serial.print("                       ");
...
/*const byte ucBigSprites2[] PROGMEM = {
  ...
  0x00, 0x00, 0xff, 0x00, 0x00, 0x28, 0x5e, 0x27,
  0x77, 0xeb, 0xcb, 0xfa, 0x77, 0x27, 0x0e, 0x00,
  0x00, 0x0c, 0x11, 0x04, 0x0c, 0x88, 0xc6, 0xe0,
  0x77, 0x26, 0x32, 0xf4, 0xe2, 0x8c, 0x0d, 0x06,
...
void DrawSprites(byte y, byte *pBuf, GFX_OBJECT *pList, byte bCount) {
  ...
  for (i = 0; i < bCount; i++) {
    ...
    bSize = (bSprite & 0x80) ? 32 : 8;//16 : 8; // big or small sprite

    // En esta operación calculamos el núm. de bytes del sprite y
    // lo multiplicamos por 2: máscara y patrón
    sSize = (pow(bSize, 2) / 8);
    ...
    if (bSize == 32) {
      // 32x32 sprite
      s = (byte *)&ucBigSprites[bSprite * 128];
      ...
      if (SCREEN_WIDTH - pObject->x < 32) { //sSize
      ...
      // simplest case
      if (bYOff == 0) {
        debugln("Pintando el bloque de 32x32");
        debugln("Valores de D: ");
        // debugln("Mascara: ");
        debugln("Pattern: ");
        for (x = 0; x < 32; x++) {
          cOld = d[0]; // Lo que hay en la pantalla en un momento dado (en un byte => 0x00)
          writeUnsignedByteHexWithPrefix(cOld); debug(", ");
          mask = pgm_read_byte(s); // Se carga la mascara para ese byte
          cNew = pgm_read_byte(s + sSize); // Se carga el patron para ese byte
          s++;
          cOld &= mask; // A lo que había en pantalla,
          cOld |= cNew;
          *d++ = cOld;

          // writeUnsignedByteHexWithPrefix(mask); debug(", ");
          // writeUnsignedByteHexWithPrefix(cNew); debug(", ");
        }

        debugln("\n------------");
        debugln("#FIN Pintando el bloque de 32x32", true);
      // only bottom half of sprite drawn
      } else if (pObject->y + 8 < y) {
        for (x = 0; x < bWidth; x++) {
          mask = pgm_read_byte(s);
          cNew = pgm_read_byte(s + sSize);
          s++;
          mask >>= (8 - bYOff); // > > =
          mask |= (0xff << bYOff);
          cNew >>= (8 - bYOff);
      ...
      // 16x16 sprite
      s = (byte *)&ucBigSprites[bSprite * sSize];
...
void setup() {
  ...
  object_list[player].y = 24;
  object_list[player].x = 24;
...
void gameLoop(void) {
  ...
  while (lives >= 0) {
    lives = -1; // hack for only one loop
    ...
    if (alienSpeed >= (14-((level-1)*5)) ) {
      ...
      if (object_list[player - 1].x >= (SCREEN_WIDTH - 8) || object_list[0].x <= 0) {
        alienX *= -1;
      }
    }
