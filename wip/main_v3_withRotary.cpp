...
const int EncoderB = 4; // PB4, pin 3
const int EncoderClick = 1; // PB1, pin 6
volatile int a0;
volatile int c0;
...
// 16x16px sprite: Guile (Street Fighter II)
...
void movePlayer(bool Right) { // Button interrupt, INT0, PB2, pin7
    ...
    if (playerDirection != oldPlayerDirection) {
      ...
      if (digitalRead(EncoderClick) == LOW) {
        ...
        if (playerDirection < oldPlayerDirection) {
          ...
          //object_list[player].x = max(0, object_list[player].x - dx);
        } else {
          // Moving sprite to right...
           object_list[player].x += dx;
          // TODO: 111 is a magic number: width screen (128px) - 16px (player width) - 1px margin
          //object_list[player].x = min(111, object_list[player].x + dx);
        }
