...
// 8x8px tiles for background testing, no mask.
const byte ucTiles[] PROGMEM = {
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55, 0xaa, 0x55,
  // --------------
  0x7e, 0xbd, 0x5b, 0xa7, 0x47, 0x8b, 0x15, 0x2a,
  0x55, 0x00, 0x55, 0xa8, 0x55, 0xa8, 0x55, 0xff,
};
...
// 16x16px sprite: Guile (Street Fighter)
...
void DrawSprites(byte y, byte *pBuf, GFX_OBJECT *pList, byte bCount) {
  byte i, x, bSize, bType, bSprite, *s, *d;
  ...
  for (i = 0; i < bCount; i++) {
    ...
    bType = pObject->bSource;
    bSize = (bSprite & 0x80) ? 16 : 8; // big or small sprite

    if (!bSprite && !bType) { // Character EOF?
      continue;
    }
    ...
    if (bSize == 16) {
      ...
      //s = (byte *)&ucFont[bSprite * 16];
      s = bType == 0
        ? (byte *)&ucSprites[bSprite * 16]
        : (byte *)&ucFont[bSprite * 8]; // 8 because no mask is needed
...
void setup() {
  ...
  // Trying to go with tiles!
  for (y = 0; y < 10; y++) { // 10 => screen Height
    d = &bPlayfield[y * 18]; // 18 => screen Width
    //c = 0; // The first index of the tiles array
    for (x = 0; x < 18; x++) { // 18 => the length of the previous array
      // *d++ = c++; // following tile (8x8px) (we need 18!) for the following array position
      c = c ^ 1; // Toggling c between 0 and 1
      *d++ = c; // Toggling the first two tiles (8x8px) for each array position
    }
    c = c ^ 1; // Toggling againg for a chess board effect
  }

  memset(object_list, 0, sizeof(object_list));

  int textLen = numberOfSprites - 2;
  // Probamos a meter una cadena:
  const char message[textLen] = { "12345678901234" }; // {'H', 'e', ... '\0'}

  // starting positions of the objects
  for (y = 0; y < message[y] != '\0'; y++) {
    object_list[y].x = (y * 7) + 8; //(y & 3) * 12;
    object_list[y].y = 0; //(y & 0xc) * 3;
    object_list[y].bType = ((int) message[y]) - 32;
    object_list[y].bSource = 1; //(y & 4) / 4; // 4 sprites, 4 letter, 4 sprites, 4 letters...
  }

  object_list[player].bType = 0x80; // 128 => big sprite
  object_list[player].x = 0;
  object_list[player].y = 36;
  object_list[player].bSource = 0;
...
void loop() {
  ...
  while (1) {
    ...
    iScrollX += (x++ & 1); // Half of speed
    // iScrollX++; // Full of speed
    // iScrollY++;
    ...
    if (object_list[player].x == 0 || object_list[player].x == 111) {
      // bounce off L/R sides
      dx = -dx;
    }

    if (object_list[player].y == 0 || object_list[player].y == 47) {
