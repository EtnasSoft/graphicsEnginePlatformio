...
void setup() {
  ...
  object_list[0].y = 48;
  iScrollX = iScrollY = 48;
}

void loop() {
  // put your main code here, to run repeatedly:
  int dx, dy, speed, currentSpeed;
  ...
  while (1) {
    ...
    iScrollX = (iScrollX >= 142) ? 0 : (iScrollX + dx); //144
    iScrollY = (iScrollY >= 78) ? 0 : (iScrollY + dy); //80
    ...
    if (analogRead(EncoderClick) < 940) {
      // Player goes up
      // object_list[0].y = max(0, object_list[0].y - 1);
      if (dy == -2) {
        dx++;
      }
